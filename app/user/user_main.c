...
#define DEVICE_ID 			"122475" //model ID, device id
...
airkiss_wifilan_time_callback(void)
{
	...
	ret = espconn_sendto(&pssdpudpconn, lan_buf, lan_buf_len);	//send udp data
	if (ret != 0) {
...
airkiss_start_discover(void)
{
	...
	espconn_create(&pssdpudpconn);	//create UDP transmission
...
smartconfig_done(sc_status status, void *pdata)
{
    switch(status) {
        ...
        case SC_STATUS_LINK:
            ...
            break;
        case SC_STATUS_LINK_OVER:	//got IP, connect to AP successfully
